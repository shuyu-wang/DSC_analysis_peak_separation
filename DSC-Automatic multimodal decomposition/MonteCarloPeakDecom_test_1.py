...
class Player(Decomer):
    ...
    def get_state(self, dealer):
        ...
        dealer_first_num_value = self.nums[0] 
        player_rmse = self.get_rmse()
        player_nums = self.total_nums()
        return dealer_first_num_value, player_nums
...
class Arena():
    ...
    def play_game(self, dealer, player):
        ...
        if player_rmse < 0.001:
            self._info("Player 2 overfitting{} lost，Score:{}\n".format(player_rmse, reward))
        ...
        if 0 < rate_rmse < 0.5:
            if selectNums == playerNums: 
                reward = 1
                self._info("Player 2 won!\n")
                self._info("The number of peaks of the decomposer should be selected：{}个, RMSE is:{}\n". format(selectNums, selectRmse))
            else:
                reward = -1
                self._info("Player 1 won!\n")
                self._info("The number of peaks of the decomposer should be selected：{}个, RMSE is:{}\n". format(selectNums, selectRmse))
        elif rate_rmse == 0:
            reward = 1
...
file = r'../dvd_dsc2.csv' # dvd protein, substract buffer
#file = r'../mab_dsc1.csv' # mab protein
#file = r'../mab_dsc2.csv' # mab protein, substract baseline
